"""Ablation study on segmentation strategy, repeat random segmentation for 20 times"""
import torch
import sys
import os
import json
...
for exp in range(EXP_NUM):
    ...
    # Random segment
    k = 8
    RESUME_SEG = random.choice(list(range(2, k, 1)))
    SEGMENTS = np.random.choice(list(range(EPOCH_START+EPOCH_PERIOD, EPOCH_END, EPOCH_PERIOD)), RESUME_SEG-1, replace=False)
    SEGMENTS.sort()
    SEGMENTS = SEGMENTS.tolist()
    SEGMENTS.insert(0, EPOCH_START)
    SEGMENTS.append(EPOCH_END)
    SEGMENTS = [(SEGMENTS[i], SEGMENTS[i+1]) for i in range(len(SEGMENTS)-1)]
    # # save time result
    save_file = os.path.join(save_dir, "segments.json")
